...
final class Buffer implements IBuffer {
	...
	private static final IThreadLocalCache<Buffer> c_bufferCache = ThreadLocalCache
			.weakLinkedCache();
	...
	static Buffer get(BufferFactory factory) {
		Buffer buffer = c_bufferCache.take();
		if (buffer == null)
			buffer = new Buffer();
	...
	public IBuffer compact() {
		...
		Unit unit = head.get();
		if (unit.isEmpty()) {
			BiListNode<Unit> next = head.next();
			if (next != head) {
				BufferFactory factory = m_factory;
				BiListNode<Unit> node = head;
	...
	private static String errMsg(int start, int length, int size) {
		return StringBuilder.get().append("start[").append(start)
				...
				.append(size).append(']').toStringAndClose();
	}

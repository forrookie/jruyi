...
public final class BigEndianShortCodec extends ShortCodec {

	public static final ShortCodec INST = new BigEndianShortCodec();
	...
	public short read(IUnitChain unitChain, int length) {
		...
		for (;;) {
			...
			if (count >= length) {
				s = getShortB(unit, position, s, length);
				unit.position(position + length);
				break;
			}
			s = getShortB(unit, position, s, count);
			unit.position(size);
	...
	public short get(IUnitChain unitChain, int index, int length) {
		...
		for (;;) {
			...
			if (count >= length) {
				s = getShortB(unit, index, s, length);
				break;
			}
			s = getShortB(unit, index, s, count);
			length -= count;
			unit = unitChain.nextUnit();
			index = 0;
		}
	...
	private static int getShortB(IUnit unit, int position, int s, final int length) {
		position += unit.start();
		int end = position + length;
		byte[] array = unit.array();
		for (; position < end; ++position)
			s = (s << 8) | (array[position] & 0xFF);

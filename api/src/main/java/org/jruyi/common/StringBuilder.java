...
public final class StringBuilder implements Serializable, Appendable,
	 ...
	 * @param len
	 *            the number of {@code char}i in the subarray to be inserted
	 * @return this object
	 * @throws IndexOutOfBoundsException
	 ...
	 *             if the offset is invalid
	 */
	public StringBuilder insertHex(int offset, byte b) {
		int newLength = m_length;
		if (offset > newLength)
			throw new IndexOutOfBoundsException();
		...
		newLength += 2;
		if (newLength > m_value.length)
			expandCapacityForInsertion(offset, 2, newLength);

		char[] v = m_value;
		int bt = b & 0xFF;
		v[offset] = c_bhDigits[bt];
		v[++offset] = c_blDigits[bt];
	 ...
	 *             if the offset is invalid
	 */
	public StringBuilder insertHex(int offset, short s) {
		int newLength = m_length;
		if (offset > newLength)
			throw new IndexOutOfBoundsException();
		...
		newLength += 4;
		if (newLength > m_value.length)
			expandCapacityForInsertion(offset, 4, newLength);
	 ...
	 *             if the offset is invalid
	 */
	public StringBuilder insertHex(int offset, int i) {
		int newLength = m_length;
		if (offset > newLength)
			throw new IndexOutOfBoundsException();
		...
		newLength += 8;
		if (newLength > m_value.length)
			expandCapacityForInsertion(offset, 8, newLength);

		char[] v = m_value;
		for (int bits = 24; bits >= 0; bits -= 8, ++offset) {
			int b = (i >> bits) & 0xFF;
			v[offset] = c_bhDigits[b];
			v[++offset] = c_blDigits[b];
		}
	 ...
	 *             if the offset is invalid
	 */
	public StringBuilder insertHex(int offset, long l) {
		int newLength = m_length;
		if (offset > newLength)
			throw new IndexOutOfBoundsException();
		...
		newLength += 16;
		if (newLength > m_value.length)
			expandCapacityForInsertion(offset, 16, newLength);

		char[] v = m_value;
		int offset2 = offset + 8;
		int i1 = (int) (l >>> 32);
		int i2 = (int) l;
		for (int bits = 24; bits >= 0; bits -= 8, ++offset, ++offset2) {
			int b = (i1 >> bits) & 0xFF;
			v[offset] = c_bhDigits[b];
			v[++offset] = c_blDigits[b];

			b = (i2 >> bits) & 0xFF;
			v[offset2] = c_bhDigits[b];
			v[++offset2] = c_blDigits[b];
		}
